...
import re
import json
from jinghuaqi.items import JinghuaqiItem
import time,random
import pandas as pd
import csv

class jdspider(scrapy.Spider):
    name="jd_jinghuaqi"                 #京东净化器
    allowed_domains=["jd.com"]
    start_urls=[
        "https://list.jd.com/list.html?cat=737,738,749"   #京东净化器
    ]
    num=0
    pagenum=0
    ...
    def goods(self,response):
        ...
        if "error" in url or "2017?t" in url:        #302重定向页面,写回原页面处理
            url="https://item.jd.com/"+str(ProductID)+".html"
            item = JinghuaqiItem(ProductID=ProductID,PreferentialPrice=PreferentialPrice,price=price)
        ...
        # --------------------全球购网页---------------------------------------------
            ...
            try:
                solid_cadr = x['固态污染物CADR'].replace('&gt;','大于')
            except:
                try:
                    solid_cadr = re.findall(">固态净化CADR：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    solid_cadr = None

            try:
                gas_cadr = x['气态污染物CADR'].replace('&gt;','大于')
            except:
                try:
                    gas_cadr = re.findall(">气态净化CADR：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    gas_cadr = None


            try:
                solid_ccm = x['固态污染物CCM（mg）'].replace('&gt;','大于')
            except:
                try:
                    solid_ccm = re.findall(">固态污染物CCM：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    solid_ccm = None

            try:
                gas_ccm=x['气态污染物CCM（mg）'].replace('&gt;','大于')
            except:
                try:
                    gas_ccm = re.findall(">气态污染物CCM：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    gas_ccm = None

            try:
                solid_level = x['固态污染物净化能效'].replace('&gt;','大于')
            except:
                try:
                    solid_level = re.findall(">净化能效：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    solid_level = None

            try:
                gas_level = x['气态污染物净化能效'].replace('&gt;','大于')
            except:
                try:
                    gas_level = re.findall(">净化能效：(.*?)<", detail_1[0])[0].strip().replace('&gt;','大于')
                except:
                    gas_level = None
            ...
            try:
                ...
                try:
                    purify = re.findall(">净化方式：(.*?)<", detail_1[0])[0].strip()
                except:
                    purify = None

            try:
                sterilization = x['除菌功能']
            except:
                try:
                    sterilization = re.findall(">除菌功能：(.*?)<", detail_1[0])[0].strip()
                except:
                    sterilization = None
            ...
            try:
                ...
                try:
                    update = re.findall(">滤网更新提醒：(.*?)<", detail_1[0])[0].strip()
                except:
                    update = None
            ...
            try:
                ...
                try:
                    sleep = re.findall(">睡眠模式：(.*?)<", detail_1[0])[0].strip()
                except:
                    sleep = None
            ...
            try:
                ...
                try:
                    autorun = re.findall(">自动模式：(.*?)<", detail_1[0])[0].strip()
                except:
                    autorun = None
            ...
            try:
                ...
                try:
                    time = re.findall(">定时模式：(.*?)<", detail_1[0])[0].strip()
                except:
                    time = None
            ...
            try:
                ...
                try:
                    windspeed = re.findall(">风扇速度设定：(.*?)<", detail_1[0])[0].strip()
                except:
                    windspeed = None

            if windspeed:
                if '不' not in windspeed and '变速' in windspeed:
                    windspeed='支持'
            ...
            try:
                ...
                try:
                    telecontrol = re.findall(">遥控模式：(.*?)<", detail_1[0])[0].strip()
                except:
                    telecontrol = None
            ...
            try:
                ...
                try:
                    aqd = re.findall(">空气质量显示：(.*?)<", detail_1[0])[0].strip()
                except:
                    aqd = None
            if aqd:
                if '不' not in aqd and '显示' in aqd:
                    aqd='支持'
        ...
        if float(PreferentialPrice)>0.00:
            ...
            item['X_name']=X_name
            item['X_type']=X_type
            item['solid_cadr'] = solid_cadr
            item['gas_cadr']=gas_cadr
            item['solid_ccm']=solid_ccm
            item['gas_ccm']=gas_ccm
            item['solid_level']=solid_level
            item['gas_level'] = gas_level
            item['purify'] = purify
            item['sterilization'] = sterilization
            item['update'] = update
            item['sleep'] = sleep
